...
const BOSS_PRESETS = {
  raid: {
    totalHp: 100000,
    parts: {
      wingL: { label: '左翼', hp: 10000, maxHp: 10000 },
      wingR: { label: '右翼', hp: 10000, maxHp: 10000 },
      head: { label: '頭', hp: 17000, maxHp: 17000 },
      tail: { label: '尻尾', hp: 13000, maxHp: 13000 },
    },
    ...
    damageMult: 120, // HP100000に合わせて1発あたりのダメージを引き上げる倍率
  },
  giant: {
    totalHp: 1000,
    parts: {
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    // 部屋コード不要のクイック参加：現在「待機中」で空きのある部屋の中から
    // 一番最近作られたものに自動で入る（コピペや招待リンクなしでワンタップ参加できるように）
    if (msg.type === 'quick_join') {
      let targetRoom = null;
      for (const r of rooms.values()) {
        if (r.state !== 'waiting') continue;
        const capacity = r.mode === 'team' ? r.teamSize * 2 : MAX_PLAYERS;
        if (r.players.size >= capacity) continue;
        if (!targetRoom || r.createdAt > targetRoom.createdAt) targetRoom = r;
      }
      if (!targetRoom) {
        sendTo(ws, { type: 'error', message: '今すぐ参加できる部屋がありません。誰かが「部屋を作成」するのを待つか、自分で部屋を作ってください。' });
        return;
      }
      playerId = genId('p');
      roomCode = targetRoom.code;
      const player = makePlayer(playerId, msg.name, targetRoom);
      player.ws = ws;
      targetRoom.players.set(playerId, player);
      sendTo(ws, { type: 'joined', playerId, roomCode: targetRoom.code, isHost: false, mode: targetRoom.mode, teamSize: targetRoom.teamSize, team: player.team });
      broadcast(targetRoom, roomSnapshot(targetRoom));
      return;
    }
